...
func _process(_delta):
	...
	if gameRunning:
		if Input.is_action_just_released("anticlockwise"):
			...
			rotateShape(true)
			drawShape(currentShape,currentPos,$tile/Live)
...
func createShape():
	...
	drawShape(currentShape,currentPos,$tile/Live)
	drawShape(nextShape,nextPos,$tile/Live)
	
func drawShape(shape,pos,layer):
	for cell in shape:
		if cell == Vector2i(1,1):
			layer.set_cell(cell+pos,0,Vector2i(1,0))
		else:
			layer.set_cell(cell+pos,0,Vector2i(0,0))
func hideDraw(shape,pos,layer):
	for cell in shape:
			layer.set_cell(cell+pos,0,Vector2i(1,1))
func eraseShape(shape,pos):
	for cell in shape:
		$tile/Live.erase_cell(cell+pos)
...
func canRotate(direction:bool):
	...
	return cr
func isFree(pos):
	return $tile/Dead.get_cell_source_id(pos)==-1
...
func landShape():
	for k in currentShape:
		eraseShape(currentShape,currentPos)
		hideDraw(currentShape,currentPos,$tile/Dead)
func checkRows():
	var row : int = 19
	while row > 0:
		var count = 0
...
func shiftRows(row):
	...
	for k in range(row, 1, -1):
		for tt in range(-1,9):
			atlas = $tile/Dead.get_cell_atlas_coords(Vector2i(tt + 1, k - 1))
#			atlas = Vector2i(0,0)
			if atlas == Vector2i(-1, -1):
				$tile/Dead.erase_cell(Vector2i(tt + 1, k))
			else:
				$tile/Dead.set_cell(Vector2i(tt + 1, k), 0, Vector2i(0,0))

func clearBoard():
	for rowY in range(20):
		for rowX in range(10):
			$tile/Dead.erase_cell(Vector2i(rowX,rowY))
	for rowX in range(15):
		for rowY in range(-5,20):
			$tile/Live.erase_cell(Vector2i(rowX,rowY))
